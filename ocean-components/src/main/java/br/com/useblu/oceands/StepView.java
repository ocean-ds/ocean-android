...
import br.com.useblu.oceands.core.ExtensionsKt;
...
public class StepView extends View {
    ...
    private boolean numberCurrentStepEnabled;
    private boolean numbersNextStepEnabled;
    ...
    private void applyStyles(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        numberCurrentStepEnabled = ta.getBoolean(R.styleable.StepView_sv_numberCurrentStepEnabled, true);
    ...
    private void drawStep(Canvas canvas, int step, int circleCenterX, int circleCenterY) {
        ...
        float strokeWidth = ExtensionsKt.getDp(2);
        ...
        if (isSelected && !isDone) {
            ...
            canvas.drawCircle(circleCenterX, circleCenterY, radius - paddingOffset, paint);

            paint.setColor(Color.WHITE);
            canvas.drawCircle(circleCenterX, circleCenterY, radius - (strokeWidth + paddingOffset), paint);
            ...
            if (state == ANIMATE_STEP_TRANSITION && step == nextAnimatedStep && nextAnimatedStep > currentStep) {
                ...
                if (nextStepCircleEnabled && nextStepCircleColor != 0) {
                    ...
                    canvas.drawCircle(circleCenterX, circleCenterY, selectedCircleRadius - paddingOffset, paint);

                    paint.setColor(Color.WHITE);
                    canvas.drawCircle(circleCenterX, circleCenterY, selectedCircleRadius - (strokeWidth + paddingOffset), paint);
                }
    ...
    private void drawLine(Canvas canvas, int startX, int endX, int centerY, boolean highlight) {
        if (highlight) {
            ...
            canvas.drawLine(startX, centerY, endX + paddingOffset, centerY, paint);
        } else {
            paint.setColor(nextStepLineColor);
            paint.setStrokeWidth(stepLineWidth);
            canvas.drawLine(startX - paddingOffset, centerY, endX + paddingOffset, centerY, paint);
        }
    }
    ...
    public class State {
        ...
        private boolean numberCurrentStepEnabled = StepView.this.numberCurrentStepEnabled;
        private boolean numbersNextStepEnabled = StepView.this.numbersNextStepEnabled;
        ...
        public State numberCurrentStepEnabled(boolean enabled) {
            this.numberCurrentStepEnabled = enabled;
            return this;
        }

        public State numbersNextStepEnabled(boolean enabled) {
        ...
        public void commit() {
            ...
            StepView.this.numberCurrentStepEnabled = numberCurrentStepEnabled;
            StepView.this.numbersNextStepEnabled = numbersNextStepEnabled;
